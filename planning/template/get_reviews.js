...
async function getRev(id) {
	try {
		let res = await fetch(`https://api.yelp.com/v3/businesses/${id}/reviews`, {
			headers: {
...
async function getReviews() {
	...
	for (let i = 0; i < houston_id.length; i++) {
		const response = await getRev(houston_id[i]);
		// reviews[houston_id[i]] = response.reviews;
		// response.business_id = houston_id[i]
		response.reviews.forEach(el => {
			el.business_id = houston_id[i]
			el.user_id = Math.floor(Math.random() * 20)
		})
